...
DROP POLICY IF EXISTS "Users can view expert availabilities for booking" ON public.expert_availabilities;
...
DROP POLICY IF EXISTS "Experts can manage their time slots" ON public.expert_time_slots;
DROP POLICY IF EXISTS "Users can view time slots for booking" ON public.expert_time_slots;
...
USING (EXISTS (
  ...
  AND expert_availabilities.expert_id::text = auth.uid()::text
))
WITH CHECK (EXISTS (
  SELECT 1 FROM public.expert_availabilities 
  WHERE expert_availabilities.id = expert_time_slots.availability_id 
