...
const TestimonialsSection = () => {
  ...
  return <section className="py-20 bg-gradient-to-b from-gray-100 to-white">
      <div className="container mx-auto px-6 sm:px-12">
        ...
        {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map(index => <div key={index} className="animate-pulse bg-white rounded-lg shadow p-6">
        ...
        }].slice(0, 4).map((testimonial, index) => <TestimonialCard key={testimonial.id || index} name={testimonial.name} location={testimonial.location} rating={testimonial.rating} text={testimonial.text} date={testimonial.date} imageUrl={testimonial.imageUrl} />)}
          </div>}
      </div>
    </section>;
};
