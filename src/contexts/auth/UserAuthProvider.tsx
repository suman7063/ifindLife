  ...
  const userAuthValue = {
    ...
    addReview: async (review: any) => {
      if (review && typeof review === 'object' && 'expertId' in review && 'rating' in review && 'comment' in review) {
        return auth.addReview ? await auth.addReview(review.expertId.toString(), review.rating, review.comment) : false;
      }
      return false;
    },
    reportExpert: async (report: any) => {
      if (report && typeof report === 'object' && 'expertId' in report && 'reason' in report && 'details' in report) {
        return auth.reportExpert ? await auth.reportExpert(report.expertId.toString(), report.reason, report.details) : false;
      }
      return false;
    },
    hasTakenServiceFrom: async (expertId: string | number) => {
      return auth.hasTakenServiceFrom ? await auth.hasTakenServiceFrom(expertId.toString()) : false;
    },
